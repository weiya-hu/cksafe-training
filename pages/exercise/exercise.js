...
Page({
  ...
  checkanswer(e){
    ...
    //只有在右上角文字不是‘下一题’和‘交卷’的时候点击才会改变切换样式的answernum（答题有问题改的第一点）
  ...
  tonext(){
    ...
    if(this.data.nexttxt==='确定'){
      console.log(that.data.sureflag)
      console.log('点击确定')
      if(that.data.sureflag){
        ...
        console.log(that.data.sureflag,'进入判断后')
        //如果这道题的填空题的时候
        if (this.data.titles[this.data.idx].belong === 1) {
          ...
          app.questUrl_noloading('jeecg-boot/wechat/question/getUserAnswer', 'post', data).then(function (res) {
            console.log(res)
            //  that.setData({sureflag: true})//把点确认答题的开关延到后面，以免后面的延时这段时间造成开关的没作用（答题有问题改的第2点）
            //如果答案正确
          ...
          app.questUrl_noloading('jeecg-boot/wechat/question/getUserAnswer', 'post', data).then(function (res) {
            console.log(res)
            //  that.setData({ sureflag: true })（答题有问题改的第3点）
            if (res.data.message === 'true') {
              that.setData({
