...
Page({
  ...
  getggao(){
    let that=this
    if (app.globalData.isshowggao){
      app.questUrl('jeecg-boot/wechat/notice', 'get').then(function (res) {
        console.log(res)
        if (res.data.success) {
          that.setData({
            isggaoshow: res.data.isExamine,
            ggaomsg: res.data.message
          })
        }
      })
    }
  ...
  closegg(){
    ...
    app.globalData.isshowggao=false
  },
  ...
  onShow: function () {
    this.getuserinfotxt()
    this.getnum()
    console.log(app.globalData.isshowggao)
    this.getheight()
    this.getggao()
  },
})
